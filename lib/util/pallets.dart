// ignore_for_file: constant_identifier_names

import 'package:flutter/material.dart';

// text
const TextStyle title = TextStyle(fontSize: 16, color: Colors.white);
const TextStyle subtitle = TextStyle(fontSize: 13, color: Colors.white);
const TextStyle text = TextStyle(fontSize: 10, color: Colors.white);
const TextStyle ctrgText = TextStyle(fontSize: 9, color: Colors.white);
const TextStyle btnText = TextStyle(
  fontSize: 15,
);

const TextStyle kBodyText = TextStyle(fontSize: 18, color: Colors.white);

const TextStyle kBodyText_login =
    TextStyle(fontSize: 16, color: Colors.white, height: 1.5);

const Color kWhite = Colors.white;
const Color kBlue = Color(0xff5663ff);
const Color kRed = Colors.lightBlue;

// Full dimensions of an action
const double ActionWidgetSize = 60.0;
const double ProfileImageSize = 40.0;

// The size of the plus icon under the profile image in follow action
const double PlusIconSize = 20.0;
